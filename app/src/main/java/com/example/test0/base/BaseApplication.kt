...
//        params.put("appId", AppConstant.APPID)
        ...
        EasyHttp.getInstance()
            ...
            //可以全局统一设置全局URL
//            .setBaseUrl(NetConstants.BASE_URL)//设置全局URL  url只能是域名 或者域名+端口号
//            .setBaseUrl("http://www.tianqiapi.com/")//设置全局URL  url只能是域名 或者域名+端口号
            ...
            .setWriteTimeOut((20 * 1000).toLong())
            .setConnectTimeout((20 * 1000).toLong())

            //可以全局统一设置超时重连次数,默认为3次,那么最差的情况会请求4次(一次原始请求,三次重连请求),
            //不需要可以设置为0
            .setRetryCount(2)//网络不好自动重试3次
            //可以全局统一设置超时重试间隔时间,默认为500ms,不需要可以设置为0
            .setRetryDelay(300)//每次延时500ms重试
